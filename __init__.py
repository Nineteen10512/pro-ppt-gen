"""PRO-PPTX v1.6.3 — PaperJSX semantic compilation architecture for professional PPT generation.
...
def _ensure_shared_alias() -> None:
    """Support absolute ``shared`` imports when installed as ``pro_ppt_gen``."""
    if "shared" in sys.modules:
        return
    try:
        sys.modules["shared"] = importlib.import_module("pro_ppt_gen.shared")
        return
    except ImportError:
        pass
    try:
...
def extract_template_theme(path: str):
    """v1.6.3: 从本地 .pptx/.potx 文件提取主题色/字体/背景 → ExtractedTheme。"""
    from shared.import_helper import import_shared
