...
# ----------------------------------------------------------------------
# v1.5.3: deep-merge helper for template theme_overrides / scanner tokens.
...
def generate(
    ...
    template_name : str | None
        v1.5.3 新增：内置预设模板名（见 pro_ppt_gen.templates.list_templates()）。
        指定后会覆盖 theme 为模板对应的 base_theme + theme_overrides + 默认 transition/animation。
    template_path : str | None
        v1.5.3 新增：本地 .pptx/.potx/.dpt 模板路径，自动提取色板/字体/背景注入 tokens。
    auto_taste_match : bool
        v1.5.3 新增：True 时若模板与内容调性匹配度低（taste_check<60）给出警告与推荐。
    """
    if output_path is None:
        output_path = _default_output_path(content, "pptx")

    # v1.5.3: resolve template_name → base_theme + overrides
    template_obj = None
    if template_name:
        try:
    ...
    # v1.5.3: apply template theme_overrides
    if template_obj is not None:
    ...
    # v1.5.3: apply local template_path theme extraction
    if template_path:
        try:
    ...
    # v1.5.3: auto_taste_match check (lightweight template-match score)
    _match_report = None
    if auto_taste_match:
        try:
...
def generate_from_markdown(
    ...
    **kwargs,
    ...
    """从 Markdown 文本直接生成 .pptx（P1-2, v1.4；v1.5.3 支持 template_* 参数）。
    ...
    Args:
        ...
        lang: 语言 ``"cn"`` | ``"en"``。
        template_name/template_path/auto_taste_match: v1.5.3 新参数，透传给 generate()。
